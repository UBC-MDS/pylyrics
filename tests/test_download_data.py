...
def test_input_types():
    ...
    with pytest.raises(TypeError):
        ...
        file_path = os.getcwd() + "/tests/data/"
        columns = ["song_title"]
        dd.download_data(dataset, "file_path", columns)  # columns: should be a list of length 2  
...
def test_dataframe():
    ...
    assert len(dd.download_data(dataset, filepath, columns).columns) == 2, "Should return a dataframe with 2 columns"
