...
def test_case2():
    try:
        lyrics = " "
        assert (
            len(ct.clean_text(lyrics).strip()) != 0
        ), "Blank text input"
    except:
        assert True
test_case1()
...
def test_case3():
    try:
        ...
        assert (
            regex.search(subtext) != None
        ), "Text cannot start with special character"
    except:
        assert True
...
def test_case4():
    try:
        lyrics = "!!I Love you"
        assert (
            len(ct.clean_text(lyrics)) <=5 
        ), "Too small to give meaniful output"
    except:
        assert True
